...
#
# Version: 1.0-beta May 4,2015 lj2007331 AT gmail.com
# Notes: LEMP stack for CentOS/RadHat 5+ Debian 6+ and Ubuntu 12+ 
#
...
#get pwd
sed -i "s@^lemp_dir.*@lemp_dir=`pwd`@" ./options.conf
...
mkdir -p $home_dir/default $wwwlogs_dir $lemp_dir/{src,conf}
...
if [ "$je_tc_malloc_yn" == 'y' -a "$je_tc_malloc" == '1' -a ! -e "/usr/local/lib/libjemalloc.so" ];then
	...
	Install_jemalloc | tee -a $lemp_dir/install.log
elif [ "$je_tc_malloc_yn" == 'y' -a "$je_tc_malloc" == '2' -a ! -e "/usr/local/lib/libtcmalloc.so" ];then
	. functions/tcmalloc.sh
	Install_tcmalloc | tee -a $lemp_dir/install.log
fi

# Database
if [ "$DB_version" == '1' ];then
	. functions/mysql-5.6.sh 
	Install_MySQL-5-6 2>&1 | tee -a $lemp_dir/install.log 
elif [ "$DB_version" == '2' ];then
        . functions/mysql-5.5.sh
        Install_MySQL-5-5 2>&1 | tee -a $lemp_dir/install.log
elif [ "$DB_version" == '3' ];then
	. functions/mariadb-10.0.sh
	Install_MariaDB-10-0 2>&1 | tee -a $lemp_dir/install.log 
elif [ "$DB_version" == '4' ];then
	. functions/mariadb-5.5.sh
	Install_MariaDB-5-5 2>&1 | tee -a $lemp_dir/install.log 
elif [ "$DB_version" == '5' ];then
        . functions/percona-5.6.sh
        Install_Percona-5-6 2>&1 | tee -a $lemp_dir/install.log
elif [ "$DB_version" == '6' ];then
	. functions/percona-5.5.sh 
	Install_Percona-5-5 2>&1 | tee -a $lemp_dir/install.log 
fi

# PHP
if [ "$PHP_version" == '1' ];then
	. functions/php-5.3.sh
	Install_PHP-5-3 2>&1 | tee -a $lemp_dir/install.log
elif [ "$PHP_version" == '2' ];then
        . functions/php-5.4.sh
        Install_PHP-5-4 2>&1 | tee -a $lemp_dir/install.log
elif [ "$PHP_version" == '3' ];then
        . functions/php-5.5.sh
        Install_PHP-5-5 2>&1 | tee -a $lemp_dir/install.log
elif [ "$PHP_version" == '4' ];then
        . functions/php-5.6.sh
        Install_PHP-5-6 2>&1 | tee -a $lemp_dir/install.log
elif [ "$PHP_version" == '5' ];then
        . functions/php-7.sh
        Install_PHP-7 2>&1 | tee -a $lemp_dir/install.log
fi

# ImageMagick or GraphicsMagick
if [ "$Magick" == '1' ];then
	. functions/ImageMagick.sh
	Install_ImageMagick 2>&1 | tee -a $lemp_dir/install.log
elif [ "$Magick" == '2' ];then
	. functions/GraphicsMagick.sh
	Install_GraphicsMagick 2>&1 | tee -a $lemp_dir/install.log
fi

# ionCube
if [ "$ionCube_yn" == 'y' ];then
        . functions/ioncube.sh
        Install_ionCube 2>&1 | tee -a $lemp_dir/install.log
fi

# PHP opcode cache
if [ "$PHP_cache" == '1' ] && [ "$PHP_version" == '1' -o "$PHP_version" == '2' ];then
        . functions/zendopcache.sh
        Install_ZendOPcache 2>&1 | tee -a $lemp_dir/install.log
elif [ "$PHP_cache" == '2' ];then
        . functions/xcache.sh 
        Install_XCache 2>&1 | tee -a $lemp_dir/install.log
elif [ "$PHP_cache" == '3' ];then
        . functions/apcu.sh
        Install_APCU 2>&1 | tee -a $lemp_dir/install.log
elif [ "$PHP_cache" == '4' -a "$PHP_version" == '2' ];then
        . functions/eaccelerator-1.0-dev.sh
        Install_eAccelerator-1-0-dev 2>&1 | tee -a $lemp_dir/install.log
...
if [ "$Nginx_version" == '1' ];then
        . functions/nginx.sh
        Install_Nginx 2>&1 | tee -a $lemp_dir/install.log
elif [ "$Nginx_version" == '2' ];then
	. functions/tengine.sh
        Install_Tengine 2>&1 | tee -a $lemp_dir/install.log
fi

# Pure-FTPd
if [ "$FTP_yn" == 'y' ];then
	. functions/pureftpd.sh
	Install_PureFTPd 2>&1 | tee -a $lemp_dir/install.log 
fi

# phpMyAdmin
if [ "$phpMyAdmin_yn" == 'y' ];then
	. functions/phpmyadmin.sh
	Install_phpMyAdmin 2>&1 | tee -a $lemp_dir/install.log
fi

# redis
if [ "$redis_yn" == 'y' ];then
	. functions/redis.sh
	Install_redis 2>&1 | tee -a $lemp_dir/install.log
fi

# memcached
if [ "$memcached_yn" == 'y' ];then
	. functions/memcached.sh
	Install_memcached 2>&1 | tee -a $lemp_dir/install.log
fi
...
if [ ! -e "$home_dir/default/index.html" -a "$Web_yn" == 'y' ];then
	. functions/test.sh
	TEST 2>&1 | tee -a $lemp_dir/install.log 
fi
